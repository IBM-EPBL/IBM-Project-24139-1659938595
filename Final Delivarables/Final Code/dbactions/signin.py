...
def get_user_id(mail_id,password):
    ...
    if conn:
        try:
            query = "SELECT MAIL_ID,PASSWORD,USER_ID,ROLE_ID FROM USERS WHERE MAIL_ID = ? AND PASSWORD = ?"
            ...
            return{ 'status' : False, 'reason' : "Something went wrong" , 'user_id' : user_id,'role_id':role_id }
        finally:
            closeConnection(conn)
        return{ 'status' : True, 'reason' : "" , 'user_id' : user_id ,'role_id':role_id}
    else:
        return{ 'status' : False, 'reason' : "Couldn't connect to DB" , 'user_id' : user_id,"role_id":role_id }
